...
import { Mail, Lock, User, Eye, EyeClosed, Loader2 } from "lucide-react";
import { useState } from "react";
import { useRouter } from "next/navigation";
import { useGoogleLogin } from "@react-oauth/google";

interface SignupProps {
  signup: (data: z.infer<typeof signupSchema>) => Promise<any>;
  login: (data: any) => Promise<any>; // Given that google auth needs to create & log in instantly
}

const signupSchema = z.object({
  name: z.string().min(2, "Name must be at least 2 characters."),
  email: z.string().email("We need a valid email to send delicious receipts."),
  password: z.string().min(6, "Password must be at least 6 characters."),
  provider: z.string(),
});

const SignupClient = ({ signup, login }: SignupProps) => {
  ...
  const onSubmit = async (signupData: z.infer<typeof signupSchema>) => {
    try {
      ...
      if (result.data || result.message?.toLowerCase().includes("success")) {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (codeResponse) => {
      try {
        setLoading(true);
        // The backend `login` endpoint naturally handles Google user creation!
        ...
        if (result.accessToken) {
          toast.success(result.message || "Signed up & logged in with Google!");
          router.push("/");
        } else {
        ...
        toast.error("An error occurred during Google auth.");
      } finally {
        setLoading(false);
      }
    },
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-linear-to-br from-orange-50 via-red-50 to-yellow-50 dark:from-orange-950/30 dark:via-red-950/30 dark:to-yellow-950/30 p-4 font-sans">
      <Card className="w-full max-w-[420px] shadow-[0_20px_60px_-15px_rgba(234,88,12,0.2)] dark:shadow-none border-orange-100 dark:border-orange-900/50 rounded-[28px] animate-in fade-in zoom-in-95 duration-500 overflow-hidden bg-white dark:bg-zinc-950">
        ...
        <CardHeader className="space-y-2 px-7 ">
          ...
          <CardDescription className="text-[15px] leading-relaxed text-gray-600 dark:text-gray-400 font-medium">
            Create an account and start ordering your absolute favorites.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="px-7 pb-8 ">
          <form
            className="space-y-4"
            onSubmit={form.handleSubmit((signupData) => onSubmit(signupData))}
          >
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <div className="space-y-1.5">
                    <Label htmlFor="name" className="font-semibold text-gray-700 dark:text-gray-300 text-[13px] uppercase tracking-wide">
                      Full Name
                    </Label>
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-12 text-[16px] font-bold transition-transform hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-orange-500/25 rounded-xl mt-3 bg-orange-600 hover:bg-orange-700 text-white border-0"
            >
          ...
          <p className="mt-6 text-center text-sm text-gray-500 dark:text-muted-foreground font-medium">
            Already have an account?{" "}
            <a href="/login" className="font-bold text-orange-600 hover:text-orange-700 dark:text-orange-500 dark:hover:text-orange-400 hover:underline underline-offset-4 transition-colors">
              Sign in
            </a>
          </p>
        </CardContent>
